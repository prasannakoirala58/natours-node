...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoicGVrYXkxMyIsImEiOiJjbDBnbzNwZTgwMGxpM2JsZWttajNtNzc2In0.jhxDJcyftS7xhTHhscIoaA';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/pekay13/cl0gpke0r000814pmn1qmjeam',
    scrollZoom: false //   center: [-118.122019, 34.104067],
    //   zoom: 4,
    //   interactive: false,
  ...
  locations.forEach(function (loc) {
    ...
    el.className = 'marker'; // Add the marker to the map

    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // Add popup of location name while hovering over the marker in the map
...
var showAlert = function showAlert(type, msg) {
  // hide all previous alerts before showing a new one
  hideAlert();
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            ...
            // dekhauna sakcha so this line is important
...
var stripe = Stripe('pk_test_51NBTVPDECaOyDfTw87zaWpoyhhJH8BR8m4tUhzCweAastrDfDpAoj1VvN7eSUp4om5upOkz6YsRJ9vAkzBHSWudW00SyQFlUha');
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 7:
            // Password update uses encryption which takes a little bit time. So we
            // provide a little feedback to the user by changing the button text
            // to 'Updating...' and then change it back to 'SAVE PASSWORD' after
            // we are done updating the passwords.
            document.querySelector('.btn--save-password').textContent = 'Save Password'; // Clearing the input fields of passwords after they have been updated
